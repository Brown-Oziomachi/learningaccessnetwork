...
export default function NotificationBell({ userId }) {
  ...
  const [readAdminReplies, setReadAdminReplies] = useState(() => {
    try {
      return new Set(
        JSON.parse(localStorage.getItem("readAdminReplies") || "[]"),
      );
    } catch {
      return new Set();
    }
  ...
  const bellRef = useRef(null); // ← on the bell button
  const dropdownRef = useRef(null); // ← on the portaled dropdown div
const [globalNotifs, setGlobalNotifs] = useState([]);
const [readGlobalIds, setReadGlobalIds] = useState(() => {
  try {
    return new Set(JSON.parse(localStorage.getItem("readGlobalNotifs") || "[]"));
...
useEffect(() => {
  ...
  // SSR guard — only render portal after hydration
  useEffect(() => {
    setMounted(true);
  }, []);

  // Outside-click: close if click is outside BOTH the bell AND the dropdown
  useEffect(() => {
    const handler = (e) => {
      if (
        dropdownRef.current &&
...
const allNotifications = [
  ...
  ...globalNotifs.map((n) => ({
    ...
    _globalId: n.id,
    id: `global__${n.id}`,
    read: readGlobalIds.has(n.id),
  })),
  ].sort((a, b) => {
    const toMs = (ts) =>
      !ts ? 0 : ts.toDate ? ts.toDate().getTime() : new Date(ts).getTime();
...
next.add(n._globalId);
  try { localStorage.setItem("readGlobalNotifs", JSON.stringify([...next])); } catch {}
  return next;
});
 } else if (n._reportId) {
  ...
  const markAllAsRead = async () => {
    try {
      setReadGlobalIds((prev) => {
  const next = new Set([...prev, ...globalNotifs.map((g) => g.id)]);
  try { localStorage.setItem("readGlobalNotifs", JSON.stringify([...next])); } catch {}
  return next;
});
      ...
      await batch.commit();
      setReadAdminReplies((prev) => {
        const next = new Set([...prev, ...repliedReports.map((r) => r.id)]);
        try {
          localStorage.setItem("readAdminReplies", JSON.stringify([...next]));
        } catch {}
        return next;
      });
    } catch (err) {
  ...
  const clearAll = async () => {
    ...
    try {
      ...
      try {
        localStorage.removeItem("readAdminReplies");
      } catch {}
    } catch (err) {
      console.error(err);
    }
  ...
  const handleBellClick = () => {
    if (!open) {
      ...
      if (rect) {
        ...
        const dropWidth = Math.min(360, vw - 32);
        let left = rect.right + 8;
        if (left + dropWidth > vw - 8) left = rect.left - dropWidth - 8;
        left = Math.max(8, left);
        let top = rect.top;
        if (top + 520 > vh - 8) top = Math.max(8, vh - 520 - 8);
  ...
  return (
    <>
      <style>{`
        ...
        .nb-bell { width:40px; height:40px; position:relative; border:0.5px solid #e5ddd0; background:#fff; display:flex; align-items:center; justify-content:center; cursor:pointer; transition:border-color .18s,background .18s; flex-shrink:0; }
        .nb-bell:hover { border-color:${GOLD}; background:${CREAM}; }
        .nb-row { display:flex; align-items:flex-start; gap:12px; padding:13px 16px; border-bottom:0.5px solid #f0ebe0; cursor:pointer; transition:background .15s; position:relative; overflow:hidden; }
        .nb-row:hover { background:${CREAM}; }
        .nb-del { width:26px; height:26px; border:0.5px solid #e5ddd0; background:transparent; display:flex; align-items:center; justify-content:center; cursor:pointer; flex-shrink:0; transition:all .15s; color:#ccc; }
        .nb-del:hover { border-color:#fecaca; background:#fef2f2; color:#dc2626; }
        @keyframes nbSlide { from { opacity:0; transform:translateX(8px) scale(.98); } to { opacity:1; transform:translateX(0) scale(1); } }
        .nb-dropdown { animation:nbSlide .22s cubic-bezier(.4,0,.2,1) both; }
        .nb-type-badge { font-size:9px; font-weight:700; letter-spacing:0.06em; padding:2px 7px; border-radius:2px; font-family:'Lato',sans-serif; }
        .nb-section-header { padding:6px 16px; background:rgba(184,150,62,0.08); border-bottom:0.5px solid rgba(184,150,62,0.2); font-size:9px; font-weight:700; letter-spacing:0.18em; text-transform:uppercase; color:${GOLD}; font-family:'Lato',sans-serif; display:flex; align-items:center; gap:6px; }
      `}</style>

      {/* Bell button — NO ref={dropdownRef} here anymore */}
      <button
        ref={bellRef}
        className="nb-bell"
      ...
      {/* Portal — only renders after hydration, escapes sidebar stacking context */}
      {open &&
        mounted &&
        createPortal(
          <div
            ref={dropdownRef} // ← ref lives HERE on the portaled div
    ...
    <div style={{
        ...
        <a href="/my-account/seller-account" onClick={() => setOpen(false)}
          ...
          document.body, // ← second argument restored — escapes sidebar stacking context
        )}
    </>
  );
}
