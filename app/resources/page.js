...
/* ══════════════════════════════════════════
   AD CARD — styled to match resource cards exactly
══════════════════════════════════════════ */
function AdResourceCard({ ad }) {
    const handleClick = () => {
        try {
    ...
    return (
        <a
            ...
            className="resource-card overflow-hidden block"
            style={{
                boxShadow: '0 2px 12px rgba(184,150,62,0.18)',
                border: '1px solid rgba(184,150,62,0.35)',
                background: '#fff',
                textDecoration: 'none',
            }}
        >
            {/* Image — same h-36 as resource cards */}
            <div className="relative overflow-hidden" style={{ height: '144px' }}>
                <img
                    ...
                    onError={e => { e.target.src = 'https://images.unsplash.com/photo-1544947950-fa07a98d237f?w=600'; }}
                />
                {/* Dark overlay */}
                <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(to top, rgba(13,34,68,0.88) 0%, transparent 55%)' }} />
                ...
                {/* GOLD SPONSOR ribbon — bottom left */}
                <div style={{
                    position: 'absolute', bottom: 8, left: 8,
                    background: 'rgba(13,34,68,0.92)', color: '#d4aa5a',
                    fontSize: '7px', fontWeight: 700, padding: '2px 8px',
            ...
            <div style={{ padding: '14px 14px 16px' }}>
                ...
                }}>{ad.title || 'Sponsored'}</h3>

                <p style={{
                    fontFamily: "'DM Sans', sans-serif",
                    fontSize: '11px', color: '#b8963e', margin: '0 0 10px',
                    overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap',
                    fontWeight: 600,
                }}>{ad.author || 'Gold Partner'}</p>

                <div style={{
                    display: 'flex', alignItems: 'center', gap: 4,
                    fontSize: '11px', fontWeight: 700,
                    color: '#0d2244', fontFamily: "'DM Sans', sans-serif",
                }}>
...
export default function ResourcesPage() {
    ...
    /* Ads — inject every 8 resource cards */
    const goldAds = useAds("Gold", 6);
    ...
    /* Inject ads into the filtered list every 8 items */
    const gridItems = injectAds(filteredTypes, goldAds, 8);
        ...
        <div className="min-h-screen" style={{ background: '#f5f2ed', fontFamily: "'Playfair Display', Georgia, serif" }}>
            <style>{`
                ...
                * { box-sizing: border-box; }
                ...
                .hero-pattern {
                    background-image:
                        radial-gradient(circle at 20% 50%, rgba(26,58,92,0.08) 0%, transparent 50%),
                ...
                .resource-card:hover { transform: translateY(-6px); box-shadow: 0 24px 48px rgba(0,0,0,0.14); }
                .resource-card .card-img { transition: transform 0.6s ease; }
                .resource-card:hover .card-img { transform: scale(1.06); }
                .filter-pill {
                    transition: all 0.2s ease; font-family: 'DM Sans', sans-serif;
                    cursor: pointer; border: 1.5px solid transparent;
                }
                .filter-pill:hover { border-color: #1a3a5c; }
                .filter-pill.active { background: #1a3a5c; color: white; border-color: #1a3a5c; }
                .upload-cta {
            ...
            {/* ── Hero ── */}
            <section ref={heroRef} className="hero-pattern grid-pattern relative pt-20 pb-16 px-4 overflow-hidden" style={{ background: '#f5f2ed' }}>
                    ...
                    <div className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-8">
                        <div className="max-w-2xl">
                            <div className="inline-block mb-4 fade-up fade-up-delay-1">
                                <span className="dm-sans text-xs tracking-[0.12em] uppercase text-blue-950 px-3 py-1.5 border border-blue-950/20"
                                    style={{ fontSize: '11px', background: 'rgba(26,58,92,0.08)' }}>
                                    Academic Library
                                </span>
                            </div>
                            <h1 className="text-5xl md:text-7xl font-black text-gray-900 leading-[0.95] mb-6 fade-up fade-up-delay-1"
                                style={{ fontWeight: 900 }}>
                                Student<br /><span style={{ color: '#1a3a5c' }}>Resources</span>
                            </h1>
                        ...
                        <div className="flex items-center gap-6 fade-up fade-up-delay-3">
                            <div className="text-center">
                                <div className="text-4xl font-black text-gray-900" style={{ fontFamily: "'DM Sans', sans-serif" }}>{documentTypes.length}</div>
                                <div className="dm-sans text-xs text-gray-500 uppercase tracking-wider mt-1">Resource Types</div>
                            </div>
                            <div className="w-px h-12 bg-gray-300" />
                            <div className="text-center">
                                <div className="text-4xl font-black text-gray-900" style={{ fontFamily: "'DM Sans', sans-serif" }}>Free</div>
            ...
            <section className="upload-cta px-4 py-8">
                <div className="max-w-7xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-4 relative z-10">
                    <div className="text-center sm:text-left">
                        <p className="text-white font-bold text-xl" style={{ fontFamily: "'Playfair Display', serif" }}>Have study materials to share?</p>
                        <p className="dm-sans text-blue-200 text-sm mt-1">Upload your documents and earn from every download.</p>
                    </div>
                    <button onClick={() => setIsUploadOpen(true)}
                        ...
                        <Upload size={16} /> Upload a Resource <ArrowUpRight size={15} />
                    </button>
                </div>
            </section>
                ...
                <div className="max-w-7xl mx-auto">
                    <div className="flex flex-col md:flex-row gap-4 items-start md:items-center">
                        <div className="relative flex-1 max-w-md">
                            ...
                            <input type="text" placeholder="Search resource types..."
            ...
            {/* ── Resources Grid (with injected ad cards) ── */}
                ...
                {filteredTypes.length === 0 ? (
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-5">
                        {gridItems.map((item, index) => {
                            /* ── AD CARD ── */
                            if (item.isAd) {
                                return <AdResourceCard key={item.id} ad={item} />;
                            }
                            ...
                            return (
                                <Link key={type.slug} href={`/document-type/${type.slug}`}
                        ...
                        <button onClick={() => setIsUploadOpen(true)}
                            ...
                            <div className="flex items-center justify-center" style={{ height: '144px', background: 'rgba(255,255,255,0.05)' }}>
                            ...
                            <div style={{ padding: '14px 14px 16px' }}>
                                <h3 className="font-bold text-white text-sm mb-1" style={{ fontFamily: "'Playfair Display', serif" }}>Upload Yours</h3>
                                <p className="dm-sans text-xs text-blue-200 leading-relaxed">Share your materials and earn from every download</p>
                                <div className="dm-sans mt-3 flex items-center gap-1 text-xs font-semibold" style={{ color: 'rgba(255,255,255,0.5)' }}>
            ...
            <section className="px-4 py-20" style={{ background: '#1a1a1a' }}>
                <div className="max-w-3xl mx-auto text-center">
                    ...
                    <button onClick={() => setIsUploadOpen(true)}
                        ...
                        <Upload size={18} /> Start Uploading <ArrowUpRight size={16} />
                    </button>
                </div>
            </section>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4"
                    ...
                    <div className="bg-white w-full max-w-md p-8 relative"
                        ...
                        <h3 className="text-2xl font-black text-gray-900 mb-2"
                            style={{ fontFamily: "'Playfair Display', serif" }}>Upload a Resource</h3>
                        ...
                        <div className="space-y-3">
                            <Link href="/uploader-agreement"
                                ...
                                <span>Upload Document</span><ArrowUpRight size={16} />
                            </Link>
                            <Link href="/become-seller"
                                ...
                                <span>Become a Seller First</span><ChevronRight size={16} />
                            </Link>
                        </div>
                        <p className="dm-sans text-xs text-gray-400 mt-6 text-center">
                            Not a seller yet?{' '}
                            <Link href="/become-seller" className="text-blue-950 underline underline-offset-2">Apply in minutes</Link>
                        </p>
                    </div>
                </div>
            )}
