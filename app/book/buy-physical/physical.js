...
import { Package, MapPin, ArrowLeft, CheckCircle, Lock, ShoppingBag, RefreshCw, Eye, EyeOff } from "lucide-react";
...
export default function BuyPhysicalClient() {
  ...
  const [step, setStep] = useState(1); // 1=confirm, 2=success, 3=existing
  ...
  // Once we have both user and bookId, check for an existing active order
  useEffect(() => {
        ...
        for (const id of variants) {
          ...
          if (!snap.empty) {
            ...
            if (active) {
              ...
              setStep(3); // show retrieval UI
              break;
            }
          }
        }
      } catch (err) {
        console.warn("Could not check existing orders:", err.message);
      }
    };
    checkExistingOrder();
  }, [user, bookId]);
  ...
  // ── Not available ────────────────────────────────────────────
  if (!book || (!inventory && step !== 3) || (inventory?.currentStock === 0 && step !== 3)) return (
  ...
  // ── Step 3: Existing order — show retrieval UI ───────────────
  if (step === 3 && existingOrder) {
    const maskedCode = codeVisible
      ? existingOrder.pickupCode
      : existingOrder.pickupCode?.replace(/./g, "•");
    ...
    return (
      <div style={{ minHeight: "100vh", background: BG, fontFamily: "'Lato',sans-serif" }}>
        ...
        <div style={{ maxWidth: "520px", margin: "32px auto", padding: "0 16px" }}>
          {/* Header */}
          <div style={{ background: NAVY, padding: "24px 28px", textAlign: "center", marginBottom: "16px" }}>
            <div style={{ width: "52px", height: "52px", background: "rgba(184,150,62,0.15)", border: `1.5px solid rgba(184,150,62,0.4)`, borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 14px" }}>
              <RefreshCw size={22} style={{ color: GOLD }} />
            </div>
            <p style={{ fontSize: "10px", fontWeight: 700, letterSpacing: "0.16em", textTransform: "uppercase", color: GOLD, margin: "0 0 6px" }}>Order Already Exists</p>
            <h1 style={{ fontFamily: "'Playfair Display',serif", fontSize: "20px", fontWeight: 700, color: "#fff", margin: "0 0 6px" }}>You've already reserved this book</h1>
            <p style={{ fontSize: "12px", color: "rgba(255,255,255,0.5)", margin: 0 }}>Retrieve your existing pickup code below</p>
          </div>

          {/* Book info */}
          <div style={{ background: "#fff", border: "0.5px solid #e5ddd0", padding: "18px 20px", marginBottom: "12px", display: "flex", gap: "14px", alignItems: "center" }}>
            {book && (
              <img src={getThumbnailUrl(book)} alt={book.title} style={{ width: "56px", aspectRatio: "3/4", objectFit: "cover", border: "0.5px solid #e5ddd0", flexShrink: 0 }} onError={e => { e.target.src = "https://images.unsplash.com/photo-1544947950-fa07a98d237f?w=400"; }} />
            )}
            <div>
              <p style={{ fontFamily: "'Playfair Display',serif", fontSize: "14px", fontWeight: 700, color: NAVY, margin: "0 0 3px", lineHeight: 1.3 }}>{existingOrder.bookTitle}</p>
              <p style={{ fontSize: "11px", color: "#888", margin: "0 0 6px" }}>by {existingOrder.bookAuthor}</p>
              <span style={{ display: "inline-flex", alignItems: "center", gap: "5px", background: existingOrder.status === "pending_pickup" ? "#fef9c3" : "#f0fdf4", border: `0.5px solid ${existingOrder.status === "pending_pickup" ? "#fde68a" : "#86efac"}`, padding: "3px 10px", fontSize: "10px", fontWeight: 700, color: existingOrder.status === "pending_pickup" ? "#a16207" : "#16a34a" }}>
                <span style={{ width: "6px", height: "6px", borderRadius: "50%", background: existingOrder.status === "pending_pickup" ? "#a16207" : "#16a34a", display: "inline-block" }} />
                {existingOrder.status === "pending_pickup" ? "Awaiting Pickup" : existingOrder.status}
              </span>
            </div>
          </div>

          {/* Pickup Code Card */}
          <div style={{ background: "#fff", border: "0.5px solid #e5ddd0", overflow: "hidden", marginBottom: "12px" }}>
            <div style={{ height: "5px", background: `repeating-linear-gradient(90deg, ${GOLD} 0, ${GOLD} 8px, transparent 8px, transparent 14px)` }} />
            <div style={{ padding: "24px" }}>
              <p style={{ fontSize: "10px", fontWeight: 700, letterSpacing: "0.16em", textTransform: "uppercase", color: "#aaa", margin: "0 0 12px", textAlign: "center" }}>Your Pickup Code</p>

              {/* Code display */}
              <div style={{ background: NAVY, padding: "20px 16px", textAlign: "center", marginBottom: "14px", position: "relative" }}>
                <p style={{ fontFamily: "'Playfair Display',serif", fontSize: "30px", fontWeight: 900, color: "#fff", margin: "0 0 4px", letterSpacing: "0.14em" }}>
                  {maskedCode}
                </p>
                <p style={{ fontSize: "10px", color: "rgba(255,255,255,0.4)", margin: 0 }}>Order ID: {existingOrder.orderId}</p>
              </div>

              {/* Reveal button */}
              <button
                onClick={() => setCodeVisible(v => !v)}
                style={{ width: "100%", background: codeVisible ? CREAM : NAVY, color: codeVisible ? NAVY : "#fff", border: `0.5px solid ${codeVisible ? "#e5ddd0" : NAVY}`, padding: "12px", fontSize: "12px", fontWeight: 700, cursor: "pointer", fontFamily: "'Lato',sans-serif", letterSpacing: "0.05em", display: "flex", alignItems: "center", justifyContent: "center", gap: "8px", transition: "all 0.18s", marginBottom: "14px" }}
              >
                {codeVisible ? <><EyeOff size={14} /> Hide Code</> : <><Eye size={14} /> Reveal Pickup Code</>}
              </button>

              {/* Order details */}
              {[
                ["Buyer", existingOrder.userName || user?.displayName || "Student"],
                ["Order Date", existingOrder.createdAt?.toDate?.()?.toLocaleDateString() || "—"],
                ["Shelf Location", [existingOrder.section, existingOrder.shelfLocation].filter(Boolean).join(" — ") || "Ask staff on arrival"],
                ["Collect At", "LAN Head Office, Abuja Registry"],
              ].map(([label, value]) => (
                <div key={label} style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", padding: "8px 0", borderBottom: "0.5px solid #f5f0e8", gap: "12px" }}>
          ...
          {/* Location note */}
          <div style={{ background: CREAM, border: "0.5px solid rgba(184,150,62,0.2)", padding: "14px 16px", marginBottom: "16px", display: "flex", alignItems: "flex-start", gap: "10px" }}>
            ...
            <div>
              ...
              <p style={{ fontSize: "11px", color: "#888", margin: 0, lineHeight: 1.55 }}>Show this code to LAN staff. You can screenshot or print this page.</p>
            </div>
          </div>
