...
export default function ReferralClient() {
    ...
    // ── Claim unclaimed earnings → add to seller wallet ────────────────────
    const handleClaim = async () => {
        ...
        try {
            ...
            // Mark all unclaimed completed referrals as claimed
            const referralsQuery = query(
                collection(db, 'referrals'),
                where('referrerId', '==', uid),
            ...
            // Use a transaction to atomically update referrals + seller wallet
            await runTransaction(db, async (transaction) => {
                // 1. Get current seller doc
                const sellerRef = doc(db, 'sellers', uid);
                const sellerDoc = await transaction.get(sellerRef);

                // 2. Mark each referral as claimed
                ...
                // 3. Add a referral withdrawal/credit record
                ...
                // 4. Update seller wallet balance & earnings
                if (sellerDoc.exists()) {
            ...
            <div className="max-w-7xl mx-auto px-4 py-12">
                        ...
                        <div>
                            ...
                            <p className="text-sm text-green-700">
                                ₦{referralStats.unclaimedEarnings > 0
                                    ? referralStats.unclaimedEarnings.toLocaleString()
                                    : referralStats.totalEarnings.toLocaleString()} has been added to your seller wallet.
                            </p>
                        </div>
                    </div>
                )}
                ...
                <div className="bg-white rounded-2xl p-6 md:p-8 shadow-sm border border-gray-200 mb-8">
                    ...
                    <p className="text-xs text-gray-400 mt-2">
                        Your unique referral ID: <span className="font-mono font-semibold text-gray-600">{referralCode}</span>
                    </p>
                </div>
