...
export default function SellersPage() {
    ...
    /* ── Navigate to profile using slug (same logic as lecturers page) ── */
  const navigateToProfile = (seller) => {
    // 1. Use stored slug if available
    if (seller.slug) {
        router.push(`/profile/${seller.slug}`);
        return;
    }

    // 2. Build name the same way the card displays it
    const displayName =
        seller.sellerName ||
        seller.businessInfo?.businessName ||
        seller.bankDetails?.accountName ||
    ...
    // 3. Strip the title out to match buildSlug(title, firstName, surname)
      const title = seller.sellerTitle || seller.title || "";
      const displayName =
          seller.sellerName ||
          seller.businessInfo?.businessName ||
          seller.bankDetails?.accountName ||
    ...
    if (nameWithoutTitle) {
        const slug = makeSlug(title, nameWithoutTitle);
        if (slug) {
                    ...
                    sellerDocs.map(async (seller) => {
                        try {
                            ...
                            if (userSnap.exists()) {
                                ...
                                // Always prefer users doc for photo and name (kept in sync by handleImageUpload/handleSave)
                                ...
                                // Keep title SEPARATE from sellerName
                                const fullName =
                                ...
                                return {
                                    ...
                                    photoURL: photo,
                                    photoBase64: photo,
                                    sellerName: fullName,        // NO title baked in
                                    sellerTitle: title,
                                    businessInfo: {
                                        ...seller.businessInfo,
                            ...
                            /* fall through to raw seller data */
                        }
                        return seller;
                    })
                );
                ...
                <section style={{
                    ...
                    <div style={{ maxWidth: 1280, margin: "0 auto", position: "relative", zIndex: 2 }}>
                        ...
                        <div style={{
                            ...
                            <Link href="/students/network" style={{
                        ...
                        <div style={{
                            ...
                            <div>
                                ...
                                <p style={{ fontSize: 13, color: MUTED, fontFamily: "'Space Grotesk', sans-serif", margin: 0, lineHeight: 1.7 }}>
                                    {sellers.length} document sellers · Browse, connect &amp; report
                                </p>
                            </div>
                        ...
                        <div style={{
                            ...
                            {[
                                { val: sellers.length, label: "Total Sellers" },
                                { val: sellers.filter(s => s.isVerifiedSeller).length, label: "Verified" },
                                ...
                                <div key={label} style={{
                                    ...
                                    <div style={{ fontFamily: "'Syne', sans-serif", fontSize: 22, fontWeight: 800, color: WHITE }}>{val}</div>
                                    <div style={{
                    ...
                    <Link href="/seller/network" className="network-banner au" style={{ marginBottom: 28, display: "flex" }}>
                        ...
                        <div style={{ display: "flex", alignItems: "center", gap: 14, flex: 1, minWidth: 0 }}>
                            <div style={{
                                ...
                                <Network size={20} style={{ color: LIME }} />
                            </div>
