      ...
      <div style={{ background: "#fff", width: "100%", maxWidth: "560px", maxHeight: "90vh", overflow: "auto", boxShadow: "0 40px 80px rgba(0,0,0,0.6)", animation: "ciSlideUp 0.3s ease both" }}>
        ...
        <div style={{ padding: "24px 28px" }}>
          ...
          <div style={{ background: "#f5f0e8", border: "0.5px solid rgba(184,150,62,0.2)", padding: "16px", marginBottom: "16px" }}>
            ...
            {[["Document Title", data.bookTitle], ["Seller", data.sellerName], ["Seller Email", data.sellerEmail], ["Quantity Consigned", `${data.totalConsignment} copies`], ["Shelf Location", data.shelfLocation || "To be assigned"], ["Course / Ref Code", data.courseCode || "—"], ["Asset ID", data.assetId]].map(([k, v]) => (
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "20px", marginBottom: "24px" }}>
            {["LAN Admin Signature", "Seller Acknowledgment"].map(l => (
              <div key={l}>
                <div style={{ borderBottom: "1.5px solid #ccc", height: "40px", marginBottom: "6px" }} />
                <p style={{ fontSize: "10px", color: "#aaa", margin: 0 }}>{l}</p>
              </div>
            ))}
          </div>
          <div style={{ display: "flex", gap: "8px", flexWrap: "wrap" }}>
            <button onClick={onClose} style={{ flex: "1 1 80px", background: "#f5f5f5", color: "#666", padding: "12px", border: "0.5px solid #e5ddd0", fontSize: "12px", fontWeight: 700, cursor: "pointer" }}>Close</button>
            <button onClick={handleWhatsApp} style={{ flex: "1 1 100px", background: "#25d366", color: "#fff", padding: "12px", border: "none", fontSize: "12px", fontWeight: 700, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: "6px" }}>
              <MessageCircle size={14} /> WhatsApp
            </button>
            <button onClick={onPrint} style={{ flex: "2 1 120px", background: "#0d2244", color: "#fff", padding: "12px", border: "none", fontSize: "12px", fontWeight: 700, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: "6px" }}>
...
/* ═══════════════════════════════════════════════════════════════
   ...
   Wrapped in an admin gate: non-admins see a locked screen
═══════════════════════════════════════════════════════════════ */
export default function AdminOfficeCheckInPage() {
  ...
  /* ── State (all check-in state lives here) ── */
  ...
  const handleSearch = async (raw) => {
    ...
    try {
      ...
      setSellerResults(rawResults.map(r => ({
        uid: r.uid, firstName: r.firstName || r.sellerName?.split(" ")[0] || "Unknown",
        surname: r.surname || r.sellerName?.split(" ").slice(1).join(" ") || "",
        email: r.email || r.sellerEmail || "—", lanAccountNumber: r.accountNumber || r.uid.slice(0, 8),
        photoBase64: r.photoBase64 || null, source: r.source,
      })));
  ...
  /* ══════════════════════════════════════════════════════════════
     ACCESS DENIED STATE — matches admin panel dark aesthetic
  ══════════════════════════════════════════════════════════════ */
    ...
    <div style={{ minHeight: "100vh", background: "#0b1628", display: "flex", alignItems: "center", justifyContent: "center", padding: "16px" }}>
      <div style={{ background: "#162033", border: "1px solid rgba(255,255,255,0.07)", borderRadius: "16px", padding: "48px 36px", maxWidth: "380px", width: "100%", textAlign: "center" }}>
        ...
        <div style={{ display: "flex", gap: "10px", flexDirection: "column" }}>
          <a href="/auth/signin" style={{ background: "#3b82f6", color: "#fff", padding: "12px 24px", borderRadius: "8px", textDecoration: "none", fontSize: "13px", fontWeight: 600, display: "block" }}>
            Sign In as Admin
          </a>
          <a href="/home" style={{ background: "transparent", color: "#64748b", padding: "10px 24px", borderRadius: "8px", textDecoration: "none", fontSize: "13px", border: "1px solid rgba(255,255,255,0.07)", display: "block" }}>
            Go Home
          </a>
        </div>
      </div>
  ...
  /* ══════════════════════════════════════════════════════════════
     MAIN ADMIN CHECK-IN — dark admin aesthetic
  ══════════════════════════════════════════════════════════════ */
  return (
    <>
      <style>{`
        ...
        :root {
          --nav-bg: #0d1b2e; --sidebar-bg: #111c2e; --card-bg: #162033;
          --card-border: rgba(255,255,255,0.07); --surface: #1a2740; --surface2: #1f2f47;
      ...
      <div className="ci-root">
        ...
        <div style={{ maxWidth: "820px", margin: "0 auto", padding: "28px 16px 80px" }}>
          ...
          <div style={{ marginBottom: "28px" }}>
            ...
            <h1 style={{ fontFamily: "'Playfair Display',Georgia,serif", fontSize: "clamp(22px,4vw,34px)", fontWeight: 700, color: "var(--text-primary)", margin: "0 0 6px" }}>
              Office Check-In
            </h1>
          ...
          {step === 1 && (
            <div className="ci-fade">
              <div className="ci-card" style={{ padding: "24px" }}>
                ...
                <p style={{ fontSize: "12px", color: "var(--text-muted)", marginBottom: "16px", lineHeight: 1.7 }}>Auto-searches as you type via 3-layer Firestore strategy. Works even with missing index fields.</p>
            ...
            <div className="ci-fade">
              <div className="ci-card" style={{ padding: "24px" }}>
                ...
                <div style={{ background: "var(--surface)", border: "1px solid var(--card-border)", borderRadius: "10px", padding: "18px", marginBottom: "18px" }}>
                  ...
                  {[["Asset ID", assetId], ["Seller", `${selectedSeller.firstName} ${selectedSeller.surname}`], ["LAN Account", selectedSeller.lanAccountNumber], ["Document", isNewPhysical ? newBookTitle : selectedBook.bookTitle], ["Course / Ref", isNewPhysical ? (newCourseCode || "—") : (selectedBook.course || "—")], ["Quantity", `${quantity} copies`], ["Shelf", shelfLocation || "To be assigned"]].map(([k, v]) => (
                ...
                <div style={{ background: "rgba(16,185,129,0.08)", border: "1px solid rgba(16,185,129,0.2)", borderRadius: "8px", padding: "12px 14px", marginBottom: "18px" }}>
                  ...
                  {["runTransaction writes to /physicalInventory (restock-aware)", "Seller receives a physical_intake notification instantly", "Receipt + WhatsApp share generated for the seller", "Admin notes saved for internal audit trail"].map((t, i) => (
                ...
                <div style={{ display: "flex", gap: "10px" }}>
                  ...
                  <button className="btn-gold" onClick={handleSubmit} disabled={submitting} style={{ flex: 1, justifyContent: "center", opacity: submitting ? 0.6 : 1 }}>
                    {submitting ? <><Loader2 size={14} style={{ animation: "ciSpin 0.8s linear infinite" }} /> Processing…</> : <><CheckCircle size={14} /> Confirm Check-In</>}
                  </button>
                </div>
              </div>
            </div>
              ...
              {recentCheckins.map((ci, i) => (
                <div key={i} style={{ background: "var(--card-bg)", border: "1px solid var(--card-border)", borderRadius: "8px", padding: "12px 14px", marginBottom: "8px", display: "flex", alignItems: "center", gap: "10px" }}>
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <p style={{ fontSize: "10px", color: "var(--text-muted)", margin: 0 }}>{ci.sellerName} · {ci.totalConsignment} copies {ci.isRestock && <span style={{ color: "var(--warning)" }}>· RESTOCK</span>}</p>
                  </div>
