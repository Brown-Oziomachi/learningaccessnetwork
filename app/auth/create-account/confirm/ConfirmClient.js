...
export default function ConfirmClient() {
    ...
    const [formData, setFormData] = useState({
        ...
        institution: '',
        institutionSlug:
            searchParams.get('institutionSlug') ||
            sessionStorage.getItem('institutionSlug') ||
            '',
        department:
            ...
            '',
        lecturerTitle:
            searchParams.get('lecturerTitle') ||
            sessionStorage.getItem('lecturerTitle') ||
            '',
        selectedUniversity:
    ...
    const handleRedirect = (role) => {
        if (role === 'seller' || role === 'lecturer') {
            router.push('/become-seller'); // lecturers become sellers
        } else {
            router.push('/student/dashboard'); // students go here
        }
    };
    ...
    const handleSubmit = async () => {
        try {
            ...
            if (result.success) {
                ...
                setTimeout(() => {
                    setShowToast(false);
                    // AFTER result.success block
                    const finalRole = userRole === 'lecturer' ? 'seller' : userRole;
...
const displayRole = userRole === 'lecturer' ? 'Seller' : userRole;
const displaySubRole =
    subRoleLabels[formData.studentSubRole] ?? null;
const detailItems = [
    {
        label: 'Account Type',
