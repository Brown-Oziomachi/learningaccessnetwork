 ...
 *    → Cost: 2 ops per bounty post (was ~1,500).
 *
...
export async function notifyPosterBountyBid(bounty, bidderName) {
    ...
    try {
        ...
        const posterSnap = await getDoc(d(db, "users", bounty.postedByUid));
        const posterEmail = posterSnap.data()?.email;
        const posterName =
            posterSnap.data()?.displayName ||
            posterSnap.data()?.firstName ||
            "Student";
