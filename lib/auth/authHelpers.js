...
// ── Generate LAN Bank account number ───────────────────────────────────────
...
export const handleGoogleSignIn = async () => {
    try {
        ...
        if (userDoc.exists()) {
            ...
            // New Google user — generate LAN account number
            await setDoc(userDocRef, {
                uid: user.uid,
                email: user.email,
                displayName: user.displayName,
                photoURL: user.photoURL,
                lanAccountNumber: generateAccountNumber(), // ✅ LAN Bank account
                status: 'active',
                role: 'student',
 ...
 * ✅ Changes:
 *    1. Added referredBy param → saved to users doc
 *    2. Added lanAccountNumber → every user gets a LAN Bank account
 *    3. Return uid → so ConfirmClient can create the referral doc
 */
export async function createUserAccount(formData) {
    try {
        ...
        // ── If seller, create Flutterwave subaccount first ──────────────────
        ...
        await setDoc(doc(db, 'users', user.uid), {
            ...
            ...(role === 'seller' && {
                ...
                rating: 0,
                reviewCount: 0,
                // ✅ Flutterwave subaccount ID saved here
        ...
        if (referredBy && referredBy !== user.uid) {
            try {
                // Find the referrer to get their UID (if referredBy is a code) 
                // OR use it directly if it's already a UID.
                // Assuming 'referredBy' passed here is the Referrer's UID:

                await addDoc(collection(db, 'referrals'), {
                    referrerId: referredBy,           // The Seller who shared the link
                    referredUserId: user.uid,         // The new person who just joined
                    referredUserName: `${firstName} ${surname}`,
                    reward: 500,                      // The 500 bonus
                    status: 'completed',
                    claimed: false,                   // So they can click 'Claim' later
                    createdAt: serverTimestamp()
                });

                console.log("Referral reward of 500 generated for:", referredBy);
            } catch (refError) {
                console.error("Error creating referral record:", refError);
                // We don't throw here so the user's signup isn't blocked if this fails
            }
        }
