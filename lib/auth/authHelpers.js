...
export async function createUserAccount(formData) {
    try {
        const {
            ...
            password,
            role = 'student',
            referredBy = null,   // this arrives as a shortCode string, not a UID
            country = '',
            studentSubRole = null,
            studyLevel = null,
            fieldOfStudy = null,
        ...
        if (resolvedReferrerId && resolvedReferrerId !== user.uid) {
            try {
                await addDoc(collection(db, 'referrals'), {
                    referrerId: resolvedReferrerId,   // ✅ real UID — matches fetchReferralStats query
                    referredUserId: user.uid,
                    referredUserName: `${firstName} ${surname}`,
                    reward: 500,
                    status: 'completed',
                    claimed: false,                   // ✅ add this so handleClaim query works
                    createdAt: serverTimestamp()
                });

                // Credit referrer's user wallet immediately
                const referrerRef = doc(db, 'users', resolvedReferrerId);
