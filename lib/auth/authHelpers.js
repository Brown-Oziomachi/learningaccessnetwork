...
export async function createUserAccount(formData) {
    try {
        ...
        const userProfile = {
            ...
            lanAccountNumber: lanAccountNumber,
            accountBalance: 0,
            referralCode: user.uid.slice(0, 8).toUpperCase(), // Optional: Give them a code
            createdAt: serverTimestamp(),
            lastLogin: serverTimestamp(),
            updatedAt: serverTimestamp(),
        ...
        if (referredBy && referredBy !== user.uid) {
            try {
                ...
                // B. Credit the Referrer's Wallet
                ...
                // We don't block the main signup if the bonus fails
            }
        }
