...
export async function claimBountyWithNotification(bountyId, userId, authUser) {
    ...
    if (bountyData.status === "pending_approval") throw new Error("BOUNTY_PENDING_APPROVAL");
    if (bountyData.status === "fulfilled") throw new Error("BOUNTY_FULFILLED");
    if (!["open", "claimed"].includes(bountyData.status)) throw new Error("BOUNTY_NOT_OPEN");
    ...
    await updateDoc(bountyRef, {
        ...
        [`bidderNames.${userId}`]: claimerName,
